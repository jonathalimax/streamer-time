...
class TwitchClient {
  ...
  /// authenticated user follows. Streams are returned sorted by number of
  /// current viewers, in descending order. Across multiple pages of results,
  /// there may be duplicate or missing streams, as viewers join and leave streams.
  ///
  ///  Required scope: [TwitchApiScope.userReadFollow]
  ///
  /// `user_id`: Results will only include active streams from the channels that
  ///  this Twitch user follows. `user_id` must match the User ID in the bearer
  ///  token.
  ///
  /// `after`: Tells the server where to start fetching the next set of results,
  /// in a multi-page response. The cursor value specified here is from the
  ...
  /// results, in a multi-page response. The cursor value specified here is from
  /// the `pagination` response field of a prior query.
  Future<TwitchResponse<TwitchStreamInfo>> getFollowedStreams({
    required String userId,
    String? after,
    String? before,
  }) async {
    final queryParameters = <String, dynamic>{'user_id': userId};
    if (after != null && after.isNotEmpty) queryParameters['after'] = after;
    if (before != null && before.isNotEmpty) queryParameters['before'] = before;

    final data = await twitchHttpClient
        .getCall(['streams', 'followed'], queryParameters: queryParameters);
    return TwitchResponse.streamsInfo(data as Map<String, dynamic>);
  }
